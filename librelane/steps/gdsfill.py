...
from gdsfill.gdsfill import check_klayout_version, fill
from gdsfill.library.common import PdkInformation, PdkNotSupportedError
...
class Filler(GdsfillStep):
    ...
    def run(self, state_in: State, **kwargs) -> Tuple[ViewsUpdate, MetricsUpdate]:
        ...
        if not check_klayout_version(pdk):
            self.err(f"KLayout version does not meet gdsfill requirements for PDK {self.config['PDK']}.")
            return {}, {}
        ...
        fill(Path(output_gds), pdk, keep_data=False, dry_run=False, max_processes=_get_process_limit())
